            ...
            <div class="card mg-b-20">
                <div class="card-body">
                    ...
                    <form class="parsley-style-1" id="selectForm2" name="selectForm2"
                        ...
                        <input type="hidden" id="client_bond_id" name="client_bond_id" value="{{ $clientBond->id }}">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-3">
                                ...
                                <select dir="rtl" id="client" required class="form-control selectpicker" data-style="btn-danger"
                                    ...
                                    @foreach($outer_clients as $client)
                                        <option value="{{ $client->client_name }}" @if($client->client_name == $clientBond->client ) selected @endif>{{ $client->client_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-3">
                            ...
                            <div class="col-md-3">
                                ...
                                <select dir="rtl" required class="form-control" id="type" name="type">
                                    <option @if( $clientBond->type == 'قبض') selected @endif>قبض</option>
                                    <option @if( $clientBond->type == 'صرف') selected @endif>صرف</option>
                                </select>
                            </div>

                            <div class="col-md-3">
                                <label> {{ __('bonds.date') }} <span class="text-danger">*</span></label>
                                <input dir="rtl" required class="form-control" id="date" name="date" type="date" value="{{$clientBond->date}}">
                            </div>
...
$(document).ready(function(){
    $("#selectForm2").submit(function(e){
        ...
        var client_bond_id = $("#client_bond_id").val();
        ...
        //start Ajax...
        $.ajax({
           url:"{{route('updateClientBond')}}",
           type:"POST",
           data:{
               company_id:company_id,
               client_bond_id:client_bond_id,
               client:client,
               account:account,
               type:type,
               date:date,
           ...
           success:function(res){
                if(res){
                    ...
                    setTimeout(function(){
                        window.location.href="/ar/client/clients-bonds/index";
                    },2000);
                }else{
                    console.log(res);
                }
